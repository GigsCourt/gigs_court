...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('users').doc(user.uid).update({
        ...
        'services': _selectedServiceNames,
      });

      if (_selectedServiceIds.isNotEmpty) {
        for (final id in _selectedServiceIds) {
          try {
            await _supabase.rpc('add_user_service', params: {'p_user_id': user.uid, 'p_service_id': int.parse(id)});
          } catch (_) {}
        }
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to save. Please try again.'), backgroundColor: AppColors.error));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: Text('Edit Profile', style: AppTextStyles.headline3)),
      body: GestureDetector(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(children: [
                    ...
                    child: SizedBox(width: 120.w, height: 120.w,
                      child: _newPhotoFile != null
                          ...
                          : photoUrl != null && photoUrl.toString().isNotEmpty
                              ...
                              : Container(color: AppColors.primary.withValues(alpha: 0.06), child: Icon(Icons.person, size: 50.sp, color: AppColors.primary.withValues(alpha: 0.3))),
                    ),
                  ),
                  Positioned(bottom: 0, right: 0, child: Container(
                    padding: EdgeInsets.all(6.w),
                    decoration: BoxDecoration(color: AppColors.primary, shape: BoxShape.circle),
                    child: Icon(Icons.camera_alt, size: 16.sp, color: AppColors.white),
                  )),
                ],
              ),
            ),
            ...
            SizedBox(height: 8.h),
            TextField(controller: _nameController, style: AppTextStyles.bodyMedium),
            SizedBox(height: 16.h),
            Text('Bio', style: AppTextStyles.label),
            SizedBox(height: 8.h),
            TextField(controller: _bioController, maxLines: 3, maxLength: 300, style: AppTextStyles.bodyMedium),
            SizedBox(height: 16.h),
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('Services', style: AppTextStyles.label),
              TextButton(onPressed: _addServices, child: Text('Edit', style: AppTextStyles.bodySmall.copyWith(color: AppColors.primary, fontWeight: FontWeight.w600))),
            ]),
            if (_selectedServiceNames.isNotEmpty)
              Wrap(spacing: 8.w, runSpacing: 8.h, children: List.generate(_selectedServiceNames.length, (i) => Chip(
                label: Text(_selectedServiceNames[i], style: AppTextStyles.bodySmall),
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              ...
              TextButton(onPressed: () => context.push('/setup/location'), child: Text('Edit', style: AppTextStyles.bodySmall.copyWith(color: AppColors.primary, fontWeight: FontWeight.w600))),
            ]),
            if (_address != null && _address!.isNotEmpty)
              Row(children: [
                Icon(Icons.location_on_outlined, size: 16.sp, color: AppColors.primary.withValues(alpha: 0.5)),
                SizedBox(width: 4.w),
                Flexible(child: Text(_address!, style: AppTextStyles.bodySmall.copyWith(color: AppColors.primary.withValues(alpha: 0.7)))),
              ]),
