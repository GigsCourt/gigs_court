...
class _ChatConversationScreenState extends State<ChatConversationScreen> {
  ...
  Future<void> _markMessagesAsRead() async {
    ...
    for (final doc in unread.docs) {
      batch.update(doc.reference, {'readAt': FieldValue.serverTimestamp(), 'status': 'read'});
    }
    await batch.commit();
    await FirebaseFirestore.instance.collection('chats').doc(widget.chatId).update({'unreadCount': 0});
  }
  ...
  Future<void> _pickAndSendPhoto() async {
    ...
    if (mounted) {
      ...
      if (result['success'] == true) {
        await _sendMessage(imageUrl: result['url']);
      }
    }
  }
  ...
  Future<void> _startRecording() async {
    ...
    await _audioRecorder.start(RecordConfig(encoder: AudioEncoder.aacLc, bitRate: 32000, sampleRate: 22050), path: path);
  }
  ...
  Future<void> _stopRecording() async {
    ...
    if (mounted) {
      ...
      if (result['success'] == true) {
        ...
        final estimatedSeconds = (fileSize / 4000).round().clamp(1, 600);
  ...
  Future<void> _addReaction(String messageId, String emoji) async {
    await FirebaseFirestore.instance.collection('chats').doc(widget.chatId).collection('messages').doc(messageId)
    ...
    final result = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(onPressed: () => Navigator.pop(ctx, true), child: Text('Save', style: TextStyle(color: AppColors.primary))),
        ],
      ),
    );
    if (result == true && controller.text.trim().isNotEmpty) {
      await FirebaseFirestore.instance.collection('chats').doc(widget.chatId).collection('messages').doc(messageId)
          .update({'text': controller.text.trim(), 'isEdited': true});
    }
  }
      ...
      builder: (ctx) => SafeArea(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Padding(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: ['👍', '❤️', '😂', '😮', '😢', '🙏'].map((emoji) {
  ...
  Future<void> _loadMoreMessages() async {
    ...
    final snapshot = await FirebaseFirestore.instance
        ...
        .startAfterDocument(_lastDocument!).limit(30).get();
    ...
    final result = await showDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) => AlertDialog(
          title: Text('Rate Provider', style: AppTextStyles.bodyLarge),
          content: Column(mainAxisSize: MainAxisSize.min, children: [
            Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(5, (i) => IconButton(
              icon: Icon(i < rating ? Icons.star : Icons.star_border, color: Colors.amber, size: 36.sp),
              onPressed: () => setDialogState(() => rating = i + 1),
            ))),
            SizedBox(height: 12.h),
            TextField(controller: controller, maxLines: 3, decoration: InputDecoration(hintText: 'Share your experience (optional)', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12.r)))),
          ]),
          actions: [
            TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancel')),
            TextButton(onPressed: () { if (rating > 0) Navigator.pop(ctx, true); }, child: Text('Submit', style: TextStyle(color: AppColors.primary))),
          ],
        ),
      ),
    );
    if (result == true && rating > 0 && _currentUser != null) {
      final existing = await FirebaseFirestore.instance.collection('reviews')
          .where('providerId', isEqualTo: widget.otherUserId).where('clientId', isEqualTo: _currentUser.uid).get();
      if (existing.docs.isNotEmpty) {
        ...
        await FirebaseFirestore.instance.collection('reviews').add({
          'providerId': widget.otherUserId, 'clientId': _currentUser.uid, 'clientName': _currentUser.displayName ?? 'Client',
          'rating': rating, 'comment': controller.text.trim(), 'createdAt': FieldValue.serverTimestamp(),
        });
      }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        leading: GestureDetector(
          onTap: (_isOtherSubscribed)
              ? () => context.push('/provider/${widget.otherUserId}')
              : null,
          child: Padding(
        ...
        title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          StreamBuilder<DocumentSnapshot>(
            ...
            builder: (context, snap) {
              ...
              return Text(typing ? 'typing...' : (_isOtherOnline ? 'Online' : 'Offline'),
                  style: AppTextStyles.caption.copyWith(color: typing ? AppColors.primary : (_isOtherOnline ? AppColors.success : AppColors.grey)));
            },
          ),
        ]),
        actions: [
          IconButton(icon: Icon(Icons.star_outline, size: 22.sp), tooltip: 'Rate Provider', onPressed: _showReviewDialog),
        ],
      ),
            ...
            child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                for (final doc in messages) {
                  ...
                  if (ts != null) {
                    final group = _getMessageGroup(ts.toDate());
                    grouped.putIfAbsent(group, () => []).add({...data, 'id': doc.id});
                  }
                }
                ...
                for (final key in keys) {
                  ...
                  for (final msg in grouped[key]!) {
                    widgets.add(_buildMessage(msg));
                  }
                }
            ...
            child: Row(children: [
              if (_isUploading)
                ...
                GestureDetector(
                  ...
                  child: Container(
                    padding: EdgeInsets.all(8.w),
                    decoration: BoxDecoration(color: _isRecording ? AppColors.error.withValues(alpha: 0.1) : Colors.transparent, borderRadius: BorderRadius.circular(24.r)),
                    child: Icon(_isRecording ? Icons.mic : Icons.mic_none, size: 24.sp, color: _isRecording ? AppColors.error : AppColors.primary),
                  ),
                ),
              ],
              SizedBox(width: 8.w),
              Expanded(
                child: TextField(
                  ...
                  onSubmitted: (t) => _sendMessage(text: t),
                ),
              ),
              SizedBox(width: 8.w),
    ...
    return Align(
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: isMine ? CrossAxisAlignment.end : CrossAxisAlignment.start, children: [
          ...
          GestureDetector(
            onLongPress: () => _showMessageMenu(msg['id'], text, type, reactions),
            child: _buildMessageBubble(type, text, imageUrl, voiceUrl, voiceDuration, isMine),
          ),
          if (reactions.isNotEmpty)
            Padding(padding: EdgeInsets.only(top: 2.h), child: Wrap(spacing: 2.w, children: reactions.map((e) => Text(e, style: TextStyle(fontSize: 14.sp))).toList())),
          SizedBox(height: 2.h),
    ...
    return FutureBuilder<DocumentSnapshot>(
      ...
      builder: (context, snap) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(color: AppColors.primary.withValues(alpha: 0.06), borderRadius: BorderRadius.circular(8.r), border: Border(left: BorderSide(color: AppColors.primary, width: 3.w))),
          child: Text(replyText, maxLines: 1, overflow: TextOverflow.ellipsis, style: AppTextStyles.caption),
        );
      },
    );
  }
  ...
  Widget _buildMessageBubble(String type, String text, String? imageUrl, String? voiceUrl, int? voiceDuration, bool isMine) {
    ...
    if (type == 'image' && imageUrl != null) {
      return GestureDetector(
        ...
        child: ClipRRect(borderRadius: BorderRadius.circular(16.r), child: CachedNetworkImage(imageUrl: imageUrl, fit: BoxFit.cover, width: double.infinity)),
      );
    }
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: bgColor, borderRadius: borderRadius, border: isMine ? null : Border.all(color: AppColors.primary.withValues(alpha: 0.1))),
  ...
  void _showFullScreenImage(String imageUrl) {
    Navigator.of(context).push(MaterialPageRoute(
      builder: (_) => Scaffold(
        ...
        body: Center(child: CachedNetworkImage(imageUrl: imageUrl, fit: BoxFit.contain)),
      ),
    ));
  }
}
...
class _VoiceBubbleState extends State<_VoiceBubble> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: bgColor, borderRadius: BorderRadius.circular(16.r), border: widget.isMine ? null : Border.all(color: AppColors.primary.withValues(alpha: 0.1))),
      child: SizedBox(
        width: width,
          ...
          Expanded(
            child: GestureDetector(
              ...
              child: ClipRRect(
                ...
                child: LinearProgressIndicator(value: progress, minHeight: 4.h, backgroundColor: textColor.withValues(alpha: 0.2), color: textColor),
              ),
            ),
          ),
          SizedBox(width: 6.w),
